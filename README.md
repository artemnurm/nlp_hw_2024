# Telegram Bot для суммаризации диалогов

Этот проект представляет собой Telegram-бота, который сохраняет сообщения пользователей и создает их краткий пересказ с помощью модели **GPT-4.1-mini** от OpenAI.

## ✨ Особенности

- 📝 **Автоматическое сохранение сообщений** в чате
- 🤖 **Суммаризация диалогов** с помощью GPT-4.1-mini
- 🌍 **Поддержка русского и английского языков**
- 📊 **Структурированный пересказ** с выделением тем и участников
- 💾 **Локальное хранение** сообщений в памяти
- 🔄 **Управление данными** (очистка, подсчет сообщений)
- 🏗️ **Модульная архитектура** с классом DialogueSummarizerBot
- ⚡ **Асинхронная обработка** сообщений

## 🚀 Быстрый старт

### Требования
- **Python 3.8+** (проверьте: `python3 --version`)
- **Токен Telegram бота** (от @BotFather)
- **API ключ OpenAI** (с platform.openai.com)

### ⚡ Пошаговая настройка

#### 1. Создание Telegram-бота

1. Найдите **@BotFather** в Telegram
2. Отправьте команду `/newbot`
3. Выберите имя и username для вашего бота
4. **Скопируйте полученный токен** - он понадобится в шаге 4

#### 2. Получение OpenAI API ключа

1. Зарегистрируйтесь на [platform.openai.com](https://platform.openai.com)
2. Перейдите в раздел **"API Keys"**
3. Создайте новый ключ
4. **Скопируйте ключ** (он показывается только один раз!)
5. **Пополните баланс** - минимум $1 для начала работы

#### 3. Подготовка проекта

```bash
# Скачайте файлы проекта или клонируйте репозиторий
# Перейдите в папку проекта
cd nlp_in_ml_1

# Установите зависимости
pip install -r requirements.txt
```

#### 4. Настройка токенов

Создайте файл `.env` в корневой папке проекта:

```bash
# Создайте файл .env
touch .env
```

Добавьте в файл `.env` следующее содержимое (замените на ваши токены):
```
TELEGRAM_BOT_TOKEN=ваш_токен_от_botfather
OPENAI_API_KEY=ваш_ключ_от_openai
```

**💡 Пример:**
```
TELEGRAM_BOT_TOKEN=123456789:ABCDEFGHIjklmnopqrstuvwxyz
OPENAI_API_KEY=sk-proj-abcdefghijklmnopqrstuvwxyz
```

#### 5. Запуск бота

```bash
python3 telegram_bot.py
```

**✅ Успешный запуск выглядит так:**
```
🚀 Запуск Telegram бота для суммаризации диалогов...
💡 Для остановки используйте Ctrl+C
📱 Найдите своего бота в Telegram и начните диалог командой /start
--------------------------------------------------
INFO - Бот запущен и готов к работе!
```

#### 6. Тестирование бота

1. **Найдите своего бота** в Telegram по username
2. **Отправьте** `/start` - должно прийти приветствие
3. **Напишите несколько сообщений** (любых)
4. **Используйте** `/count` - должно показать количество сообщений
5. **Используйте** `/summarize` - получите пересказ диалога

### 🔧 Возможные проблемы

| Проблема | Решение |
|----------|---------|
| `ModuleNotFoundError` | Установите зависимости: `pip install -r requirements.txt` |
| Бот не запускается | Проверьте файл `.env` и токены |
| Ошибка OpenAI API | Проверьте баланс на OpenAI аккаунте |
| Бот не отвечает | Убедитесь что токен от @BotFather правильный |

---

## 🎯 Команды бота

| Команда | Описание |
|---------|----------|
| `/start` | Показать приветствие и список команд |
| `/summarize` | Создать пересказ всех сохранённых сообщений |
| `/clear` | Очистить все сохранённые сообщения |
| `/count` | Показать количество сохранённых сообщений |

## 💡 Как использовать

1. **Запустите бота** командой `python3 telegram_bot.py`
2. **Найдите бота в Telegram** и начните диалог командой `/start`
3. **Напишите несколько сообщений** - бот автоматически их сохранит
4. **Используйте `/summarize`** для получения пересказа диалога
5. **Управляйте данными** с помощью `/clear` и `/count`

## 📊 Пример работы

**Ваши сообщения:**
```
Привет! Как дела?
Сегодня обсуждали новый проект
Нужно до пятницы подготовить презентацию
Кто будет отвечать за дизайн?
```

**Пересказ от бота:**
```
📄 Пересказ диалога (4 сообщения)

🔸 Основные темы: Приветствие, обсуждение нового проекта, подготовка презентации
🔸 Ключевые моменты: Презентацию нужно подготовить до пятницы
🔸 Участники: Пользователь инициировал обсуждение проекта и задал вопрос о дизайне
🔸 Нерешённые вопросы: Кто будет отвечать за дизайн презентации

🤖 Создано с помощью GPT-4.1-mini
```

## ⚙️ Технические детали

### Структура проекта
```
nlp_in_ml_1/
├── telegram_bot.py          # Основной код бота
├── requirements.txt         # Зависимости Python
├── QUICK_START.md          # Быстрый старт
├── README.md               # Эта документация
└── .env                    # Переменные окружения (создать самостоятельно)
```

### Используемые технологии
- **Python 3.8+**
- **python-telegram-bot>=20.0** - для работы с Telegram API
- **OpenAI Python SDK>=1.0.0** - для работы с GPT-4.1-mini
- **python-dotenv>=1.0.0** - для загрузки переменных окружения
- **nest-asyncio>=1.5.0** - для асинхронной работы

### Архитектура

Бот построен с использованием класса `DialogueSummarizerBot`, который включает:

- **Хранение сообщений**: `chat_messages` - словарь для хранения сообщений по чатам
- **Обработчики команд**: методы для каждой команды бота
- **Форматирование**: метод `_format_messages_for_summary()` для подготовки данных
- **API интеграция**: метод `_get_summary()` для работы с OpenAI
- **Обработка ошибок**: централизованный `error_handler()`

### О модели GPT-4.1-mini
- **Контекст:** До 1,047,576 токенов (~750,000 слов)
- **Стоимость:** $0.42 за 1M входных токенов, $1.68 за 1M выходных
- **Качество:** Сопоставимо с GPT-4o при значительно меньшей стоимости
- **Особенности:** Отличные способности к программированию и следованию инструкциям

### Конфигурация

Настройки можно изменить в `telegram_bot.py`:

```python
# Максимальное количество сообщений для хранения
MAX_MESSAGES_PER_CHAT = 100

# В методе _get_summary():
max_tokens=1000,      # Длина ответа GPT
temperature=0.3       # Креативность модели (0.0-1.0)
```

### Ограничения
- **Максимум сообщений:** 100 на чат (настраивается через `MAX_MESSAGES_PER_CHAT`)
- **Только текст:** Обрабатывает только текстовые сообщения
- **Временное хранение:** Данные хранятся только во время работы бота
- **Интернет:** Требует подключения для работы с API

## 🔧 Отладка и управление

### Логирование

Бот ведет подробные логи:
```
2024-01-01 12:00:00 - __main__ - INFO - Сохранено сообщение от username в чате 123456
2024-01-01 12:01:00 - __main__ - INFO - Бот запущен и готов к работе!
```

### Тестирование подключения

Для проверки работы OpenAI API можно добавить тестовую функцию:

```python
async def test_openai_connection():
    """Тест подключения к OpenAI API"""
    try:
        response = client.chat.completions.create(
            model="gpt-4.1-mini",
            messages=[{"role": "user", "content": "Привет!"}],
            max_tokens=10
        )
        print("✅ OpenAI API работает:", response.choices[0].message.content)
    except Exception as e:
        print("❌ Ошибка OpenAI API:", e)
```

### Остановка бота

Для корректной остановки используйте `Ctrl+C`. Бот перехватывает сигнал и завершается корректно.

## 💰 Стоимость использования

**Примерная стоимость одной суммаризации:**
- Короткий диалог (10 сообщений): ~$0.001
- Средний диалог (50 сообщений): ~$0.003
- Длинный диалог (100 сообщений): ~$0.005

**Месячное использование** (50 суммаризаций): ~$0.15

## 🔒 Безопасность

- ✅ Токены хранятся в `.env` файле (не в коде)
- ✅ Логирование без персональных данных пользователей
- ✅ Обработка ошибок API с graceful degradation
- ✅ Валидация входных данных
- ⚠️ **Важно:** Добавьте `.env` в `.gitignore` перед коммитом
- ⚠️ **Важно:** Не передавайте конфиденциальную информацию через бота

## 🤝 Поддержка и решение проблем

### Частые проблемы

**Бот не запускается:**
1. Проверьте наличие файла `.env` с корректными токенами
2. Убедитесь, что установлены все зависимости: `pip install -r requirements.txt`
3. Проверьте версию Python: должна быть 3.8+

**Бот не отвечает на команды:**
1. Убедитесь, что токен Telegram корректный
2. Проверьте, что бот запущен от @BotFather
3. Попробуйте перезапустить бота

**Ошибки суммаризации:**
1. Проверьте баланс на OpenAI аккаунте
2. Убедитесь в корректности OpenAI API ключа
3. Проверьте интернет-соединение

### Получение помощи

Если проблема не решается:
1. Проверьте логи в консоли при запуске бота
2. Убедитесь, что все переменные окружения установлены корректно
3. Попробуйте минимальный пример подключения к API

## 📝 Лицензия

Этот проект создан в образовательных целях. Используйте ответственно и в соответствии с Terms of Service OpenAI и Telegram.

## 🔗 Полезные ссылки

- [Документация python-telegram-bot](https://docs.python-telegram-bot.org/)
- [OpenAI API Documentation](https://platform.openai.com/docs)
- [Создание Telegram ботов с BotFather](https://core.telegram.org/bots/tutorial) 